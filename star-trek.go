...
func main() {
	...
	var copyOfTheWorldMap = WorldMap
	...
	for {
		...
		for {
			...
			if WarpFactor > MaxWarpFactor || WarpFactor < 1 {
		...
		if CurrentPosition+WarpFactor > len(WorldMap)-1 {
			WorldMap[CurrentPosition] = TypeWorldMap[CurrentPosition]
			CurrentPosition = len(WorldMap) - 1
			WorldMap[CurrentPosition] = WorldMap[CurrentPosition] + "P"
			WorldMap[CurrentPosition-1] = copyOfTheWorldMap[CurrentPosition-1]
			WarpFactor = 0
		} else {
			WorldMap[CurrentPosition] = TypeWorldMap[CurrentPosition]
